...
职责说明：
- 输入：接收检索器返回的候选结果以及原始查询。
- 处理：基于标题匹配、标签匹配、关键词密度、时效性和长度等信号重新打分。
- 输出：返回按相关性重新排序后的 RerankedResult 列表。
- 下游传递：重排序后的结果会回到检索流水线，作为最终送入生成器的上下文候选。
- 依赖数据：主要使用检索结果里的 text、metadata、score，以及原始查询文本本身。
"""
